    ...
    >
      ...
      <Reveal delay={100} easing="ease">
        ...
        <div className="mb-8.5 flex divide-x divide-line">
          {stats.map((s, i) => (
            <div key={s.label} className="flex-1 px-3 first:pl-0">
              ...
              <div className="mt-2 font-mono text-2xs leading-[1.4] text-mute">{s.label}</div>
            </div>
          ))}
        </div>
        <div className="flex flex-wrap items-end gap-4.5">
          ...
          <span className="pb-1.5 font-mono text-2xs text-mute">fundador · ainda faz code review na sexta</span>
        </div>
      </Reveal>
    </Section>
  );
