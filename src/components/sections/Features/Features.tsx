        ...
        <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
          {features.map((ft, i) => (
            <Reveal key={ft.kicker} delay={(i % 3) * 60} duration={600} easing="ease">
              <div className="h-full rounded-[0.875rem] border border-line bg-ink-800 p-7 transition-colors hover:border-[#3B342A]">
                <div className="mb-5 font-mono text-[0.6875rem] tracking-[0.14em] text-gold">{ft.kicker}</div>
                <h3 className="mb-2.5 text-xl font-semibold tracking-[-0.01em]">{ft.title}</h3>
                <p className="text-[0.9375rem] leading-[1.6] text-mute">{ft.body}</p>
              </div>
            </Reveal>
          ))}
        </div>
