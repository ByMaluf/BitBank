...
const providers: { key: ProviderRate; icon: React.ReactNode; label: React.ReactNode }[] = [
  {
    key: "padrao",
    icon: <i className="fa-brands fa-aws text-sm text-mute" />,
  ...
  {
    key: "padrao",
    icon: <i className="fa-brands fa-microsoft text-xs-mono text-mute" />,
      ...
      <div className="relative mx-auto flex w-full max-w-[75rem] flex-1 flex-col justify-center px-8 py-10">
        <div className="mb-14 grid items-start gap-[4.5rem] lg:grid-cols-[1fr_1.1fr]">
          <Reveal easing="ease">
            ...
            <p className="mb-9 max-w-[44ch] text-body leading-[1.6] text-mute">
            ...
            <div
              className="flex items-center justify-between gap-4 rounded-btn border border-line bg-ink-800 py-4 pr-5 pl-5 transition-colors"
              style={{ borderLeft: `3px solid ${info.borda}` }}
            >
              <div>
                <div className="mb-1.5 font-mono text-3xs tracking-[0.14em] text-mute-2">CARTÃO ATIVO</div>
                <span className="text-body font-bold tracking-[-0.01em] transition-colors" style={{ color: info.cor }}>
                  {info.nome}
                </span>
              </div>
              <span className="max-w-[26ch] text-right font-mono text-xs-mono text-sub">{info.rota}</span>
            </div>
          </Reveal>
        ...
        <Reveal delay={150} easing="ease">
          <div className="mb-2.5 ml-0.5 font-mono text-2xs tracking-[0.1em] text-mute">
          ...
          <div className="grid grid-cols-2 gap-2.5 md:grid-cols-4">
            {providers.map((p, i) => (
              <div
                key={i}
                className="rounded-btn border border-white/10 bg-white/5 px-5 py-4.5 shadow-[inset_0_1px_0_rgba(255,255,255,0.08)] backdrop-blur-md"
              >
                ...
                <div className="mt-1.5 flex items-center gap-1.5">
                  {p.icon}
                  <div className="font-mono text-xs-mono text-mute">{p.label}</div>
                </div>
              </div>
            ))}
          </div>
